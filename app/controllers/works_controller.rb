    ...
    if params[:edit_button]
      ...
      if valid && @work.set_challenge_info && @work.save
        ...
        if params[:preview_button] || params[:cancel_coauthor_button]
          flash[:notice] = ts('Draft was successfully created. It will be <strong>automatically deleted</strong> on %{deletion_date}.', deletion_date: view_context.date_in_zone(@work.created_at + 1.month, 'UTC')).html_safe
